...
// If defined, it is called on each items of the select and should return a boolean for whether or not the item fits the searched term.
type Searcher func(input string, index int) bool

// NotFound is an index returned when no item was selected.
// This could happen due to a search without results.
const NotFound = -1

// List holds a collection of items that can be displayed with an N number of visible items.
// The list can be moved up, down by one item of time or an entire page (ie: visible size). It keeps track of the current selected item.
type List struct {
	items    []*interface{}
	scope    []*interface{}
...
// The items attribute must be a slice type with a size greater than 0.
// Error will be returned if those two conditions are not met.
...
// If the selected item is out of view, the new select item becomes the last visible item.
// If the list is already at the top, nothing happens.
func (l *List) Prev() {
	if l.cursor > 0 {
		l.cursor--
	}
...
// The list must implement the searcher function signature for this functionality to work.
...
// If the selected item is out of view, the new select item becomes the first visible item.
// If the list is already at the bottom, nothing happens.
func (l *List) Next() {
	max := len(l.scope) - 1
...
// PageUp moves the visible list backward by x items.
// Where x is the size of the visible items on the list.
// The selected item becomes the first visible item.
// If the list is already at the bottom, the selected item becomes the last visible item.
func (l *List) PageUp() {
...
// PageDown moves the visible list forward by x items.
// Where x is the size of the visible items on the list.
// The selected item becomes the first visible item.
